function [pupil, glint, targets, Rpc] = crsLiveTrackGet9PointFixationDataHID(deviceNumber, viewDist, screenSize, NoOfGlints, Window1ID, Window2ID)
...
% y-pos=down)
% calTargets = [-5 -5;5 -5;5 5;-5 5;0 0]; 
% calTargets = [-7 -7;7 -7;7 7;-7 7;0 0]; %UL, UR, DR, DL, Ctr
calTargets = [-7 -7;0 -7;7 -7;-7 0;0 0;7 0;-7 7;0 7;7 7];
% calTargets = [0 -5;0 5;5 0;-5 0;0 0];
...
w = PsychImaging('OpenWindow', Window2ID, 0);
...
for i = 1:size(calTargets,1)
    ...
    Screen('FillOval', w, 255, round([tgtLocs(i,1)-fixRad...
...
%         R = LiveTrackHIDGetRawSample(deviceNumber,1);
    ...
    while ~gotFix,
        ...
        Screen('FillOval', w, 255, round([tgtLocs(i,1)-fixRad tgtLocs(i,2)-fixRad tgtLocs(i,1)+fixRad tgtLocs(i,2)+fixRad]));
        Screen('Flip', w);
