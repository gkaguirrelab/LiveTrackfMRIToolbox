function [subject,subjectDataDir,saveFileName] = OLGetSubjectDataDirMR(protocolDataDir,protocolList,protocolIndex,scaleCalibrationFlag)
% [subject,subjectDataDir,saveFileName] = GetSubjectDataDir(protocolDataDir,protocolList,protocolIndex,scaleCalibrationFlag)
%
...
while true
    ...
    elseif any(subjectIndex == 1:length(availableSubjects))
        ...
        if scaleCalibrationFlag
            subjectDataDir = sprintf('%s/%s/%s/%s/%s', protocolDataDir, ...
                subject, datestr(now, 'mmddyy'), 'EyeTrackingFiles', 'ScaleCalibration');
        else
